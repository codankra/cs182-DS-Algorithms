 ...
 * Also, do NOT use the HeapBottomUp method in the textbook to reorder your queue. That is for creating a heap, 
 * not reordering an existing one. It probably could work, but it has a horrible time complexity
 * compared to the correct solution (Which isn't that difficult, just requires some thought). 
 * Perhaps even some of the logic of HeapBottomUp carries over...
 */
 ...
 * Why the position? Think Prim's where we update a key in our Priority Queue.
...
struct handle {
   ...
   * This is NOT a function. You do NOT implement this in minprio.h
   ...
   * and passes it into the constructor, and then we save a 
