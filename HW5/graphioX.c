...
void writeGraph(GraphInfo gi) {
    int count = 0;
    printf("%i\n", gi->graph->numVerts());
    while(count<gi->graph->numVerts()){
        printf("%s\n", gi->vertnames[count]);     
        count++;
    }
    count = 0;
    while(count<gi->graph->numVerts()){
        int* su = gi->graph->successors(count);
        int suCount = 0;
        while (su[suCount] != -1){
            printf("%s", gi->vertnames[count]);
            printf("%s", " ");
            printf("%s", gi->vertnames[su[suCount]]);
            printf("%s", " ");
            printf("%f\n", gi->graph->edge(count, su[suCount]));
            suCount++;        
        }
        count++;
    }
